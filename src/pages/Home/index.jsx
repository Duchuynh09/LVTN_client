...
function Home() {
  ...
  useEffect(() => {
    for (let index = 0; index < data.length; index++) {
      if (data[index].mssv === idsv) {
        const sv = document.querySelector(`#${idsv}`);
        if (document.querySelector(".activeSeat")) {
          document.querySelector(".activeSeat").classList.remove("activeSeat");
        }
        sv.classList.add("activeSeat");
        ModalContextt.setMess(
          `Vị trí ghế ngồi của bạn là ${sv.firstElementChild.attributes.name.value}`
        );
        ...
        ModalContextt.setMess(
          "Không tìm thấy ghế ngồi, vui lòng kiểm tra lại!"
        );
